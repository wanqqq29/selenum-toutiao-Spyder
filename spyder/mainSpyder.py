...
from spyder.models import artical as ar
...
def spyder(groupID):
    ...
    for i in List:
        ...
        if len(i.get_attribute('href')) !=0:
            ...
            arSql.href=i.get_attribute('href')
            content = contReq(i.get_attribute('href'))
            if len(content) != 0:
                arSql.content = content
            else:
                arSql.content = ' '
        else:
            href.append(' ')
    ...
    return jsonStr  #返回json格式化后的数据，直接return出去
